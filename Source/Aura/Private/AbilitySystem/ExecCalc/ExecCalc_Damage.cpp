	...
	FGameplayEffectCustomExecutionOutput& OutExecutionOutput) const
{
	...
	for (const auto& Pair : FAuraGameplayTags::Get().DamageTypesToResistances)
	{
		...
		float DamageTypeValue{ Spec.GetSetByCallerMagnitude(Pair.Key) }; // GetSetByCallerMagnitude will return 0.f if damage type not found
		
		float Resistance{ 0.f };
		ExecutionParams.AttemptCalculateCapturedAttributeMagnitude(CaptureDef, EvaluationParameters, Resistance);
		Resistance = FMath::Clamp(Resistance, 0.f, 100.f);

		// Reduce damage type value by target's resistance to that type of damage
		DamageTypeValue *= (100.f - Resistance) / 100.f;
		// Add damage type value to total damage
